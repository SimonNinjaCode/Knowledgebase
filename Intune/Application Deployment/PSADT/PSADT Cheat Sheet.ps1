...
## Uninstallation Location in the Registry
...
## Intune Install & Uninstall Command
Deploy-Application.exe 
Deploy-Application.exe -DeploymentType "Uninstall"

## Local Test
.\Deploy-Application.ps1 -Verbose
.\Deploy-Application.ps1 -Deploymenttype "Uninstall"

## View Uninstall from the Registry
Get-ChildItem -Path HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall, HKLM:\SOFTWARE\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall 
| Get-ItemProperty | Select-Object -Property DisplayName, UninstallString
...
Start-Process -FilePath '<application>.exe' -Argumentlist '/quiet'

## *** Execute Process as the active user***
Execute-ProcessAsUser -Path "$PSHOME\powershell.exe" -Parameters "-Command & { & `"$Dirfiles\Script.ps1`"; Exit `$LastExitCode }" -Wait
...
Remove-File -Path "$envCommonDesktop\EXAMPLE.lnk"
