...
$CustomerId = "a3b6d4f4-ea43-4e83-9899-19958eac5d8c"  

# Replace with your Primary Key
$SharedKey = "vv/SiLycDyvBNWf+WH0edEkeFon2lPNoMccI4Xp3uHFGnlJ27Hx4GuLtkDkJiCpjpY3s6xtybB2534tMkLgW2w=="
...
$IPConfiguration = Get-NetIPConfiguration -InterfaceIndex $CurrentNetAdapter.ifIndex
...
Post-LogAnalyticsData -customerId $customerId -sharedKey $sharedKey -body ([System.Text.Encoding]::UTF8.GetBytes($json)) -logType $DeviceLog | Out-Null
...
$json = $AppArray | ConvertTo-Json

# Submit the data to the API endpoint
Post-LogAnalyticsData -customerId $customerId -sharedKey $sharedKey -body ([System.Text.Encoding]::UTF8.GetBytes($json)) -logType $AppLog | Out-Null
