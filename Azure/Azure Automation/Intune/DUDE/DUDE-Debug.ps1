...
Write-Output "Loading Function - Invoke-GraphCall..."
...
try {
    ...
    if ($null -eq $StorageAccount) {
        Write-Error "Could not get storage account: $StorageAccount"
    }
}
catch {
    Write-Error "Error connecting to Azure or getting storage account: $($_.Exception.Message)"
}
...
try {
    ...
    if ($null -eq $Table) {
        Write-Error "Could not get AzTable: $TableName"
    }
    $TableContent = Get-AzTableRow -Table $Table
    Write-Output "TableContent = $($TableContent.Count)"
}
catch {
    Write-Error "Error getting table or table content: $($_.Exception.Message)"
}
#endregion

#region Get GraphAccessToken
Write-Output "Get GraphAccessToken..."
try {
